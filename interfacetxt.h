...
string makeStringOf(GeneralFact* obj)
{
	...
	case 'm':
	{
		...
		else if ("module_counter" == ALine)
		{
			module_counter* ptr = dynamic_cast<module_counter*>(obj);
			ss << "module_counter(" << ptr->q;
...
GeneralFact* makeInstanceOf(string inputline)
{
	...
	return new last_while_loop_entry(q);
	}
		break;
	}
	case 'm':
...
factstar makefactstar(string inputline)
{
	...
	case 'l':
	{
		...
		else if (ALine == "last_while_loop_entry")
		{
			...
			ptr = new last_while_loop_entry(q);
		}
		break;
	}
	case 'm':
...
size_t matchfactsSpec(GeneralFact* Treesfact, factUnderInspection* obj) // node of a tree, fact with underscores
{
	...
	case 'm':
	{
		...
		{
		module_counter* ptr = dynamic_cast<module_counter*>(fact);
		module_counter* ptr2 = dynamic_cast<module_counter*>(Treesfact);
		if (!tcmp.empty())
		{
			if (tcmp[0])
				if (ptr->q != ptr2->q) return 0;
